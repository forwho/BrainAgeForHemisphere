...
def train(train_loader, model, first_stage_model,criterion1, criterion2, optimizer, device, epoch):
    ...
    for i, (img,_,target, male) in enumerate(train_loader):
        ...
        if opt.lr_loss:
            limg = img.clone()
            limg[:,:,int(limg.size(0)/2):,:,:] = 0
            rimg = img.clone()
            rimg[:,:,0:int(rimg.size(0)/2),:,:] = 0

        input = img.to(device)
        if opt.lr_loss:
            linput = limg.to(device)
            rinput = rimg.to(device)
        ...
        if opt.lr_loss:
                lpredicted_residual_age, loutput_age = model(linput,male,dis_age)
                rpredicted_residual_age, routput_age = model(rinput,male,dis_age)
        ...
        if opt.lr_loss:
            lloss1 = criterion1(lpredicted_residual_age, target_residual_age)
            if opt.lbd > 0:
                lloss2 = criterion2(loutput_age, target)
            else:
                lloss2 = 0
            lloss = lloss1 + opt.lbd * lloss2

            rloss1 = criterion1(rpredicted_residual_age, target_residual_age)
            if opt.lbd > 0:
                rloss2 = criterion2(routput_age, target)
            else:
                rloss2 = 0
            rloss = rloss1 + opt.lbd * rloss2

            lrloss = criterion1(loutput_age, routput_age)
            lwloss = criterion1(loutput_age, output_age)
            rwloss = criterion1(routput_age, output_age)

            loss = loss + lloss + rloss + lrloss + lwloss + rwloss

        mae = metric(output_age.detach(), target.detach().cpu())
        if opt.lr_loss:
            lmae = metric(
                loutput_age.detach(), target.detach().cpu())
            rmae = metric(
                routput_age.detach(), target.detach().cpu())
            lwmae = metric(loutput_age.detach(), output_age.detach())
            rwmae = metric(routput_age.detach(), output_age.detach())
            lrmae = metric(loutput_age.detach(), routput_age.detach())
            mae = (mae + lmae + rmae + lwmae + rwmae + lrmae)/6
        losses.update(loss, img.size(0))
        LOSS1.update(loss1,img.size(0))
        LOSS2.update(loss2,img.size(0))
        MAE.update(mae, img.size(0))
        if opt.lr_loss:
            LMAE.update(lmae, img.size(0))
            RMAE.update(rmae, img.size(0))
            LWMAE.update(lwmae, img.size(0))
            RWMAE.update(rwmae, img.size(0))
            LRMAE.update(lrmae, img.size(0))
            LLOSS1.update(lloss1,img.size(0))
            LLOSS2.update(lloss2,img.size(0))
            RLOSS1.update(rloss1,img.size(0))
            RLOSS2.update(rloss2,img.size(0))
        ...
        if i % opt.print_freq == 0:
            ...
            if opt.lr_loss:
                print(
                  'LLoss1 {LLOSS1.val:.3f} ({LLOSS1.avg:.3f})\t'
                  'LLoss2 {LLOSS2.val:.3f} ({LLOSS2.avg:.3f})\t'
                  'LMAE {LMAE.val:.3f} ({LMAE.avg:.3f})\t\n'
                  'RLoss1 {RLOSS1.val:.3f} ({RLOSS1.avg:.3f})\t'
                  'RLoss2 {RLOSS2.val:.3f} ({RLOSS2.avg:.3f})\t'
                  'RMAE {RMAE.val:.3f} ({RMAE.avg:.3f})\t\n'
                  'LWMAE {LWMAE.val:.3f} ({LWMAE.avg:.3f})\t'
                  'RWMAE {RWMAE.val:.3f} ({RWMAE.avg:.3f})\t'
                  'LRMAE {LRMAE.val:.3f} ({LRMAE.avg:.3f})\t'.format
                  ( epoch, opt.epochs, i, len(train_loader)
                  , LLOSS1=LLOSS1, LLOSS2=LLOSS2, LMAE=LMAE, RLOSS1=RLOSS1, RLOSS2=RLOSS2, RMAE=RMAE, LWMAE=LWMAE, RWMAE=RWMAE, LRMAE=LRMAE ))
...
def validate(valid_loader, model, first_stage_model,criterion1,criterion2, device):
    ...
    with torch.no_grad():
        for _, (input,_,target,male) in enumerate(valid_loader):
            ...
            if opt.lr_loss:
                linput = input.clone()
                linput[:,:,int(linput.size(0)/2):,:,:] = 0
                rinput = input.clone()
                rinput[:,:,0:int(rinput.size(0)/2),:,:] = 0


            input = input.to(device)
            if opt.lr_loss:
                linput = linput.to(device)
            ...
            # =========== compute output and loss =========== #
            predicted_residual_age, output_age = model(input, male, dis_age) #TODO
            target_residual_age = target - dis_age
