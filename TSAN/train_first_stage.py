...
def train(train_loader, model, criterion1, criterion2, optimizer, device, epoch):
    ...
    for i, (img,_,target, male) in enumerate(train_loader):
        ...
        if opt.model == 'ScaleDense':
            ...
            if opt.lr_loss:
                lout = model(linput,male)
                rout = model(rinput,male)
        ...
        if opt.lr_loss:
            lloss1 = criterion1(lout, target)
            if opt.lbd > 0:
                lloss2 = criterion2(lout, target)
            else:
                lloss2 = 0
            lloss = lloss1 + opt.lbd * lloss2

            rloss1 = criterion1(rout, target)
            if opt.lbd > 0:
                rloss2 = criterion2(rout, target)
            else:
                rloss2 = 0
            rloss = rloss1 + opt.lbd * rloss2

            lrloss = criterion1(lout, rout)
            lwloss = criterion1(lout, out)
            rwloss = criterion1(rout, out)
        ...
        if opt.lr_loss:
            lmae = metric(
                lout.detach(), target.detach().cpu())
            rmae = metric(
                rout.detach(), target.detach().cpu())
            lwmae = metric(lout.detach(), out.detach())
            rwmae = metric(rout.detach(), out.detach())
            lrmae = metric(lout.detach(), rout.detach())
